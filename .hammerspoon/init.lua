...
-- Trackpad swipe gestures (three-finger everywhere, two-finger in Slack).
...
-- mapped keystroke:
--   - three-finger left/right = previous/next tab (Slack: previous/next unread)
--   - three-finger down       = close tab
--   - two-finger left/right   = back/forward (Slack only)
--
-- During a real swipe the reported touch count fluctuates (resting thumb,
-- transient extra contacts), so we latch onto one finger by identity once an
-- eligible finger count is down and keep tracking it until every finger lifts.
-- Left/right fire once per stroke and again on a reversal (so a back-and-forth
-- swipe repeats); the destructive three-finger down swipe fires only once per
-- gesture.
--
...
--
-- Two-finger swipes are also recognized, but only while Slack is frontmost:
-- Slack's Electron UI doesn't support macOS's native two-finger swipe-to-navigate,
-- so we emulate back/forward there. Everywhere else a two-finger swipe is left
-- alone (e.g. normal scrolling).
local threeFingerCount = 3
local twoFingerCount = 2
...
-- Returns true if Slack is the frontmost application.
local function isSlackFrontmost()
  local app = hs.application.frontmostApplication()
  return app ~= nil and app:bundleID() == slackBundleID
end

-- Returns true if the given touch count should be allowed to latch a new swipe.
-- Three fingers are always eligible; two fingers are only eligible in Slack.
local function isEligibleFingerCount(count)
  if count == threeFingerCount then
    return true
  end
  return count == twoFingerCount and isSlackFrontmost()
end
...
local swipeState = {
  tracking = false,
  trackedId = nil,
  -- Finger count that latched the current gesture (2 or 3); fixes which
  -- keystrokes fire and rejects the gesture if the count changes mid-swipe.
  fingerCount = nil,
  startX = 0,
  startY = 0,
  lastFireTime = 0,
...
--
-- Three-finger left/right normally switches tabs (Cmd+Shift+[ / Cmd+Shift+]);
-- in Slack it instead jumps to the previous/next unread channel (Slack's own
-- shortcut), which frees two-finger swipes for Cmd+[ / Cmd+] (back/forward).
local function sendSwipeKey(direction, fingerCount)
  if fingerCount == twoFingerCount then
    -- Matches macOS's natural-scrolling convention (content follows the
    -- fingers): swiping right reveals earlier content (back), swiping left
    -- reveals later content (forward).
  ...
  if direction == "left" then
    if isSlackFrontmost() then
      eventtap.keyStroke({ "shift", "alt" }, "up", 0)
    else
      eventtap.keyStroke({ "shift", "cmd" }, "[", 0)
    end
  elseif direction == "right" then
    if isSlackFrontmost() then
      eventtap.keyStroke({ "shift", "alt" }, "down", 0)
    else
      eventtap.keyStroke({ "shift", "cmd" }, "]", 0)
    end
...
local function maybeFireTap()
  ...
  -- Only a three-finger gesture counts as a tap; two-finger taps (Slack only)
  -- are not mapped to anything.
  if swipeState.fingerCount ~= threeFingerCount then
    return
  end
...
-- Handles gesture events: latches onto one finger once an eligible finger
-- count is down (three, or two in Slack), fires the mapped keystroke when the
-- tracked finger crosses the threshold on the dominant axis, and resets once
...
local function handleGesture(eventObject)
  ...
  -- or the count otherwise deviates from the one that started the gesture.
  -- Block re-latching until every finger lifts, while preserving tracking so
  -- the release path can still decide whether the gesture was a tap.
  if swipeState.tracking and count ~= swipeState.fingerCount then
    swipeState.blockedUntilRelease = true
    return false
  end

  if not swipeState.tracking then
    if isEligibleFingerCount(count) then
      local identity, position = firstTouching(touches)
      if identity ~= nil then
        swipeState.tracking = true
        swipeState.trackedId = identity
        swipeState.fingerCount = count
  ...
  elseif swipeState.fingerCount == threeFingerCount and deltaY <= -swipeThreshold then
    direction = "down"
  end
  ...
  if direction ~= nil then
    sendSwipeKey(direction, swipeState.fingerCount)
