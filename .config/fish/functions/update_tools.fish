function update_tools --on-event fish_prompt
    ...
    if test -z "$last_run_timestamp"; or test (math "$current_timestamp - $last_run_timestamp") -ge $min_interval_seconds
        ...
        if not test -f $lock_file
            ...
            set -l tmux_command "__dotfiles_update_tools; set -l exit_code \$status; if test \$exit_code -eq 0; echo 'Update completed successfully'; else; echo 'Update failed (exit code: '\$exit_code')'; end; read -n1 -P 'Press any key to close...'; tmux kill-pane"
